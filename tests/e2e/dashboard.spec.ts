...
test.describe('Dashboard - Protected Route (Unauthenticated)', () => {
  ...
  test('should redirect unauthenticated user to login page', { tag: ['@smoke', '@regression'] }, async ({ dashboardPage }) => {
  ...
  test('should redirect to login if accessing detail page while not authenticated', { tag: ['@regression'] }, async ({
    page,
  }) => {
...
test.describe('Dashboard - Authenticated User', () => {
  ...
  test('should display dashboard after successful login', { tag: ['@smoke', '@regression'] }, async ({ dashboardPage }) => {
  ...
  test('should redirect to login after logout', { tag: ['@regression'] }, async ({ dashboardPage }) => {
...
test.describe('Dashboard - Header', () => {
  ...
  test('should display React Demo App title in header', { tag: ['@regression'] }, async ({ dashboardPage }) => {
    await expect(dashboardPage.headerTitle).toBeVisible();
    await expect(dashboardPage.headerTitle).toHaveText('React Demo App');
  });

  test('should display user email in header', { tag: ['@regression'] }, async ({ dashboardPage }) => {
  ...
  test('should NOT display admin badge for regular user', { tag: ['@regression'] }, async ({ dashboardPage }) => {
...
test.describe('Dashboard - Admin User', () => {
  ...
  test('should display admin badge for admin user', { tag: ['@regression'] }, async ({ dashboardPage }) => {
    await expect(dashboardPage.adminBadge).toBeVisible();
    await expect(dashboardPage.adminBadge).toHaveText('Admin');
  });

  test('should display admin email in header', { tag: ['@regression'] }, async ({ dashboardPage }) => {
    await expect(dashboardPage.userEmail).toBeVisible();
    const userEmail = await dashboardPage.getUserEmail();
...
test.describe('Dashboard - Product List', () => {
  ...
  test('should display 4 products', { tag: ['@smoke', '@regression'] }, async ({ dashboardPage }) => {
  ...
  test('should display all product names', { tag: ['@regression'] }, async ({ dashboardPage }) => {
    for (const product of testProducts) {
      await expect(
        dashboardPage.page.getByTestId(`product-link-${product.id}`)
      ).toBeVisible();
    }
  });
...
test.describe('Dashboard - Sorting', () => {
  ...
  test('should sort products by name A-Z', { tag: ['@smoke', '@regression'] }, async ({ dashboardPage }) => {
  ...
  test('should sort products by price low to high', { tag: ['@regression'] }, async ({ dashboardPage }) => {
    await dashboardPage.selectSortOption('price-asc');
    const names = await dashboardPage.getProductNames();
    expect(names).toEqual(sortedByPriceAsc);
  });

  test('should sort products by price high to low', { tag: ['@regression'] }, async ({ dashboardPage }) => {
...
test.describe('Dashboard - Product Navigation', () => {
  ...
  test('should navigate to product detail when clicking product name', { tag: ['@smoke', '@regression'] }, async ({
    dashboardPage,
    productDetailPage,
  }) => {
  ...
  test('should navigate to product detail using product link', { tag: ['@regression'] }, async ({
    dashboardPage,
    productDetailPage,
  }) => {
  ...
  test('should navigate to product detail when clicking product image', { tag: ['@regression'] }, async ({
    dashboardPage,
    productDetailPage,
  }) => {
...
test.describe('Product Detail Page', () => {
  ...
  test('should display product name', { tag: ['@regression'] }, async ({ dashboardPage, productDetailPage }) => {
    await dashboardPage.clickProduct(testProducts[0].name);
  ...
  test('should display product price', { tag: ['@regression'] }, async ({ dashboardPage, productDetailPage }) => {
    await dashboardPage.clickProduct(testProducts[0].name);
  ...
  test('should display product image', { tag: ['@regression'] }, async ({ dashboardPage, productDetailPage }) => {
    await dashboardPage.clickProduct(testProducts[0].name);
    await expect(productDetailPage.productImage).toBeVisible();
  });

  test('should display product description', { tag: ['@regression'] }, async ({ dashboardPage, productDetailPage }) => {
    await dashboardPage.clickProduct(testProducts[0].name);
    await expect(productDetailPage.productDescription).toBeVisible();
  });

  test('should navigate back to product list when clicking back button', { tag: ['@regression'] }, async ({
    dashboardPage,
    productDetailPage,
  }) => {
