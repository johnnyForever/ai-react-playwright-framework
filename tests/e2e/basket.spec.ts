...
test.describe('Shopping Basket', () => {
  ...
  test.describe('Basket Icon', () => {
    test(
      'should display basket icon in header',
      { tag: ['@regression'] },
      async ({ dashboardPage }) => {
    ...
    test(
      'should not show count badge when basket is empty',
      { tag: ['@regression'] },
      async ({ dashboardPage }) => {
    ...
    test(
      'should navigate to checkout when clicking basket icon',
      { tag: ['@regression'] },
  ...
  test.describe('Add to Basket - Dashboard', () => {
    test(
      'should add product to basket from dashboard',
    ...
    test(
      'should show correct count when adding multiple products',
      { tag: ['@regression'] },
      async ({ dashboardPage }) => {
    ...
    test(
      'should remove product from basket when clicking Remove from Basket',
      { tag: ['@smoke', '@regression'] },
      async ({ dashboardPage }) => {
        await test.step('Add product to basket', async () => {
          await dashboardPage.addToBasket(testProducts[0].id);
    ...
    test(
      'should prevent adding same product twice',
      { tag: ['@regression'] },
      async ({ dashboardPage }) => {
        await dashboardPage.addToBasket(testProducts[0].id);
  ...
  test.describe('Add to Basket - Product Detail', () => {
    test(
      'should add product to basket from product detail page',
      { tag: ['@regression'] },
    ...
    test(
      'should remove product from basket on product detail page',
      { tag: ['@regression'] },
      async ({ dashboardPage, productDetailPage }) => {
        await dashboardPage.clickProductById(testProducts[0].id);
    ...
    test(
      'should sync basket state between dashboard and product detail',
      { tag: ['@regression'] },
      async ({ dashboardPage, productDetailPage }) => {
...
test.describe('Checkout Page', () => {
  ...
  test.describe('Empty Checkout', () => {
    test(
      'should show empty basket message when no items',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
    ...
    test(
      'should show continue shopping button when empty',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
    ...
    test(
      'should navigate back to dashboard when clicking continue shopping',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
  ...
  test.describe('Checkout with Items', () => {
    ...
    test(
      'should display items in checkout',
      { tag: ['@smoke', '@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
    ...
    test(
      'should display product details in checkout',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
    ...
    test(
      'should display correct total quantity',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
    ...
    test(
      'should display basket icon on checkout page',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
  ...
  test.describe('Remove from Checkout', () => {
    ...
    test(
      'should remove item from checkout',
      { tag: ['@smoke', '@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await test.step('Navigate to checkout', async () => {
          await dashboardPage.goToCheckout();
    ...
    test(
      'should update basket count when removing item',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
    ...
    test(
      'should update total when removing item',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
    ...
    test(
      'should show empty message when all items removed',
      { tag: ['@regression'] },
      async ({ dashboardPage, checkoutPage }) => {
        await dashboardPage.goToCheckout();
  ...
  test.describe('Checkout - Protected Route', () => {
    ...
    test(
      'should redirect to login if accessing checkout while not authenticated',
      { tag: ['@regression'] },
      async ({ page }) => {
  ...
  test.describe('Order Confirmation', () => {
    ...
    test(
      'should display finish order button on checkout',
      { tag: ['@regression'] },
      async ({ checkoutPage }) => {
    ...
    test(
      'should navigate to order confirmation when clicking finish order',
      { tag: ['@smoke', '@regression'] },
    ...
    test(
      'should display thank you message on order confirmation',
      { tag: ['@smoke', '@regression'] },
      async ({ checkoutPage, page }) => {
    ...
    test(
      'should display return to dashboard button',
      { tag: ['@regression'] },
      async ({ checkoutPage, page }) => {
        await checkoutPage.finishOrder();
    ...
    test(
      'should navigate back to dashboard when clicking return button',
      { tag: ['@regression'] },
      async ({ checkoutPage, page }) => {
        await test.step('Complete order', async () => {
          await checkoutPage.finishOrder();
    ...
    test(
      'should clear basket after order confirmation',
      { tag: ['@smoke', '@regression'] },
      async ({ checkoutPage, dashboardPage, page }) => {
        await test.step('Complete order', async () => {
          await checkoutPage.finishOrder();
        });
    ...
    test(
      'should display order number on confirmation',
      { tag: ['@regression'] },
      async ({ checkoutPage, page }) => {
        await checkoutPage.finishOrder();
